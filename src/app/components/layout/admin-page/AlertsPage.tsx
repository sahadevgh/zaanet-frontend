        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            {alertsData?.alerts?.offline?.map((alert, index) => (
              <div key={`offline-${index}`} className="flex items-start p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="w-2 h-2 bg-gray-500 rounded-full mt-2 mr-3"></div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-gray-900">{alert.networkId || "Unknown"}</span>
                    <span className="text-sm text-gray-600">{new Date(alert.timestamp).toLocaleTimeString()}</span>
                  </div>
